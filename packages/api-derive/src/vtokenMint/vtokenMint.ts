...
export function getPoolInfo(
  instanceId: string,
  api: ApiInterfaceRx
): (tokenId: number, preBlockHash?: BlockHash) => Observable<convertPool> {
  return memo(instanceId, (tokenId: number, preBlockHash?: BlockHash) => {
    ...
    return tokenInfo.pipe(
      mergeMap((tokenInfo) => {
        if (tokenInfo.tokenType === 2) {
          // if it is a token

          if (preBlockHash === undefined) {
            result = api.query.vtokenMint.pool(tokenId);
          } else {
            result = api.query.vtokenMint.pool.at(preBlockHash, tokenId);
          }
 ...
 * @param api
 */
export function getAllVtokenConvertInfo(
  instanceId: string,
  api: ApiInterfaceRx
): (tokenArray?: number[]) => Observable<convertPool[]> {
 ...
 * @param api
 */
export function getConvertPriceInfo(
  instanceId: string,
  api: ApiInterfaceRx
): (tokenId: number, preBlockHash?: BlockHash) => Observable<number> {
 ...
 * @param api
 */
export function getAllConvertPriceInfo(
  instanceId: string,
  api: ApiInterfaceRx
): (tokenArray?: number[]) => Observable<number[]> {
  return memo(instanceId, (tokenArray?: number[]) => {
    let tokenList: Observable<number[]>;
...
export function getAnnualizedRate(instanceId: string, api: ApiInterfaceRx): (tokenId: number) => Observable<number> {
  return memo(instanceId, (tokenId: number) => {
    ...
    const historicalPrice$ = preBlockHashQuery().pipe(
      mergeMap((preHash) => {
        // mergeMap operator is used to flatten the two levels of Observables into one.
    ...
    return combineLatest([currentPrice$, historicalPrice$]).pipe(
 ...
 * @param api
 */
export function getAllAnnualizedRate(
  instanceId: string,
  api: ApiInterfaceRx
): (tokenArray?: number[]) => Observable<number[]> {
  return memo(instanceId, (tokenArray?: number[]) => {
...
export function getBatchConvertPrice(
  instanceId: string,
  api: ApiInterfaceRx
): (tokenId: number, blockHashArray: Observable<BlockHash[]>) => Observable<number[]> {
  return memo(instanceId, (tokenId: number, blockHashArray: Observable<BlockHash[]>) => {
    const getConvertPriceInfoQuery = getConvertPriceInfo(instanceId, api);

    return blockHashArray.pipe(
      mergeMap((blockHashList) => {
        return combineLatest(
          blockHashList.map((blockHash) => {
