...
export function getDesignatedBlockHash(
  instanceId: string,
  api: ApiInterfaceRx
): (numBlockBack?: number) => Observable<BlockHash> {
...
export function calCurrentDateHourZeroBlockHeight(
  instanceId: string,
  api: ApiInterfaceRx
): () => Observable<timestampListAndBlockHeight> {
  return memo(instanceId, () => {
    const BLOCK_INTERVAL = 6; // 6 seconds to generate a block
    const getHeaderQuery = getHeader(instanceId, api);
...
export function generateBachBlockHeightList(
  instanceId: string,
  api: ApiInterfaceRx
): (intervalBlocks?: number, totalNumber?: number) => Observable<timestampListAndBlockHeightList> {
